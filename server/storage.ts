...
export class MemStorage implements IStorage {
  ...
  private initializeSampleData() {
    ...
    const sampleProduct2: Product = {
      id: "product-2",
      name: "Fresh Lettuce",
      productType: "Leafy Greens",
      quantity: 50,
      farmLocation: "Sunny Acres Farm, Oregon",
    ...
    const sampleProduct3: Product = {
      ...
      productType: "Root Vegetables",
      quantity: 75,
      farmLocation: "Mountain View Farm, Colorado",
    ...
    this.products.set(sampleProduct3.id, sampleProduct3);
    ...
    // Add supply chain steps for other products
    const additionalSteps: SupplyChainStep[] = [
      {
        id: "step-5",
        productId: "product-2",
        stage: "farm",
        ...
        transactionHash: "0x2b3c4d5e6f7890abcdef1234567890abcdef1234",
        qualityMetrics: { temperature: 2, humidity: 70, pH: 6.5 },
      },
      {
        id: "step-6",
        productId: "product-3",
        ...
        company: "Mountain View Farm",
        status: "completed",
        timestamp: new Date(Date.now() - 86400000), // 1 day ago
        transactionHash: "0x3c4d5e6f7890abcdef1234567890abcdef123456",
        qualityMetrics: { temperature: 3, humidity: 60, pH: 6.8 },
      },
    ];

    additionalSteps.forEach(step => this.supplyChainSteps.set(step.id, step));
    ...
    sampleTransactions.forEach(tx => {
      this.blockchainTransactions.set(tx.transactionHash, tx);
    });
  }
  ...
  async getBlockchainTransaction(hash: string): Promise<BlockchainTransaction | undefined> {
    return Array.from(this.blockchainTransactions.values()).find(tx => tx.transactionHash === hash);
  }

  async getRecentTransactions(limit: number = 10): Promise<BlockchainTransaction[]> {
    const transactions = Array.from(this.blockchainTransactions.values());
    return transactions
      .sort((a, b) => {
